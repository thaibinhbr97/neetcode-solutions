// // time: O(r * c)
// // space: O(r * c), for visited and queue
// class Solution {
//     int dr[4] = {-1, 1, 0, 0};
//     int dc[4] = {0, 0, -1, 1};
// public:
...
//         return (r >= 0 && r < rows && c >= 0 && c < cols);
//     }
//     int bfs(int sr, int sc, int rows, int cols, vector<vector<int>>& grid) {
//         queue<pair<int, int>> q;
//         q.push({sr, sc});
//         grid[sr][sc] = 0;
//         int area = 1;
...
public:
    ...
    int dfs(int sr, int sc, int rows, int cols, vector<vector<int>>& grid, int area) {
        if (!isValid(sr, sc, rows, cols) || grid[sr][sc] == 0) return area;
        grid[sr][sc] = 0;
        area++;
        for (int i = 0; i < 4; i++) {
            area = max(area, dfs(sr + dr[i], sc + dc[i], rows, cols, grid, area));
        }
        return area;
    }
    int maxAreaOfIsland(vector<vector<int>>& grid) {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                if (grid[r][c] == 1) {
                    maxArea = max(maxArea, dfs(r, c, rows, cols, grid, 0));
                }
            }
        }
        return maxArea;
